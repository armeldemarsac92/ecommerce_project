...
public class TagRepository : ITagRepository
{
    ...
    {
        var response = await _dbCommand.QueryAsync<TagSQLResponse>(TagQueries.GetAllTags, cancellationToken);
    ...
    {
        return await _dbCommand.QuerySingleAsync<TagSQLResponse>(TagQueries.CreateTag, createTagRequest, cancellationToken);
    }

    public async Task<int> UpdateAsync(UpdateTagSQLRequest updateTagRequest, CancellationToken cancellationToken = default)
    { 
        return await _dbCommand.ExecuteAsync(TagQueries.UpdateTag, updateTagRequest, cancellationToken);
    }

    public async Task<int> DeleteAsync(long productTagId, CancellationToken cancellationToken = default)
    {
        return await _dbCommand.ExecuteAsync(TagQueries.DeleteTag, new { TagId = productTagId}, cancellationToken);
    }
}